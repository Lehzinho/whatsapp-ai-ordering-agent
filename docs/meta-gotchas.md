# WhatsApp Cloud API — the errors that actually stop a setup

Everything here was hit and solved on a real setup, not copied from the docs.
If you are debugging a stuck WhatsApp integration, start at the symptom.

---

## "My webhook is verified, but I never receive a message"

The most common one, and the docs bury it.
...
webhook** arrives seconds later with `status: failed` and this error. Easy to miss if you
...
The test number is for development only. It cannot be relied on for cross-country delivery.
...
Keep the raw value too — memory keys and dedupe should use one form consistently.

---

## The SMS verification code never arrives

Common in Brazil and on several carriers worldwide. Don't keep re-requesting it.
...
re-register the number later, and there is no self-service recovery.

---

## `This phone number is already registered on a WhatsApp account`

The Cloud API only accepts numbers with **no active WhatsApp account**. Having "stopped
using WhatsApp" on that number is not enough — the account still exists on Meta's side.
...
- **The test token expires in 24 hours.** Fine for a first test, useless in production.
...
- **Identity-linked API keys.** Some Anthropic keys are scoped to a workspace and require an
...
When something doesn't work, check in this order — it goes from cheapest to most expensive:
...
4. **Does a status webhook arrive right after, and what does it say?** ← most answers live here
